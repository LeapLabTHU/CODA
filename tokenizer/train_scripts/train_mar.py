...
def log_validation(
    ...
    for image in config.validation_images:
        ...
        model_input = vae.encode(pixel_values).sample().mul_(0.2325)

        _, latents, _, _ = multi_quantizer(model_input)

        image = vae.decode(latents / 0.2325)
...
def main(config):
    ...
    vae =  MARAutoencoderKL(
        embed_dim=16, ch_mult=(1, 1, 2, 2, 4), ckpt_path=config.pretrained_vae_path
    ) # hard code mar params
    ...
    def save_model_hook(models, weights, output_dir):
        ...
        if accelerator.is_main_process:
            for i, model in enumerate(models):
                ...
                if isinstance(model, MARAutoencoderKL):
    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                model_input = vae_wo_ddp.encode(pixel_values).sample().mul_(0.2325)
                model_input = model_input.to(dtype=weight_dtype)

                _, latents, vq_loss, entropy_loss = multi_quantizer(model_input)

                images = unwrap_model(vae).decode(latents / 0.2325)

                loss, log_g = criterion(
                    pixel_values.float(), images.float(), 
                    optimizer_idx = 0,
                    last_layer = unwrap_model(vae).decoder.up[-1].upsample.conv.lora_layer.up.weight, # change to last peft out layer
